*! did_switching: Treatment effect estimation and pre-trend testing in staggered adoption diff-in-diff designs with an imputation approach, modified to support treatment switching on and off.
*! Version: June 3, 2024
*! Original Author: Kirill Borusyak
*! Modifications by: David Murphy, Floris Zoutman
*! Original Version: November 22, 2023
*! Original Citation: Borusyak, Jaravel, and Spiess, "Revisiting Event Study Designs: Robust and Efficient Estimation" (2023)
*! Modified Version Citation: Borusyak, Jaravel, and Spiess (2023), with modifications by [Your Name] (2023)

cap program drop did_switching
program define did_switching, eclass sortpreserve
    version 13.0
    syntax varlist(min=5 max=5) [if] [in] [aw iw] [, wtr(varlist) sum Horizons(numlist >=0) ALLHorizons HBALance HETby(varname) PROject(varlist numeric) ///
    ...
    qui {
        ...
        local D `4'
        local K `5'
        markout `touse' `Y' `t' `D' `K' // markout the new variables
        // Process FE
        if ("`fe'"=="") local fe `i' `t'
        if ("`fe'"==".") {
            tempvar constant
        ...
        if (`delta'<=0 | mi(`delta')) {
            di as error "A problem has occurred with determining delta. Please specify it explicitly."
            error 198
        }

        if (`debugging') noi di "#1"
        *gen `K' = (`t'-`D'+`shift')/`delta' if `touse'
        ...
        *gen `D' = (`K'>=0 & !mi(`K')) if `touse'
        ...
        if ("`avgeffectsby'"=="") local avgeffectsby = "`D' `t'"
    ...
    * Horizons
    ...
    count if (`D'==0) & (`touse')
    if (r(N)==0) {
        if (`shift'==0) noi di as error "There are no untreated observations, i.e. those with `t'<`K' or mi(`K')."
            else noi di as error "There are no untreated observations, i.e. those with `t'<`K'-`shift' or mi(`K')."
    ...
    * Compare model degrees of freedom [does not work correctly for timecontrols and unitcontrols, need to recompute]
    ...
    ereturn local cmd did_switching
